...
function ShoppingCart() {
  const { cart, totalOrderPrice, totalOrderProducts } = useSelector(
    (state) => state.cart
  );
    ...
    >
      ...
      >
        {cart.map((it) => (
          ...
          >
            <OrderButtons>
              <RemoveButton handleClick={() => onRemoveElement(it.name)} />
            </OrderButtons>
          </CardResume>
        ))}
      </OrderCard>
      {openModal && <Modal orderNumber={idOrder} onContinue={goHome} />}
    </ShoppingCartLayout>
  );
}
